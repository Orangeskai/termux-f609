...
/**
 * Library from walangkaji
 * And remake and recode by Orangeskai
 * @modder Orangeskai (https://github.com/Orangeskai)
 * @author walangkaji (https://github.com/walangkaji)
 */

//all stuff YOU SHOULD NOT TOUCH IT !!!//
...
class ZteApi
{
    ...
    public function login()
    {
        ...
        $postdata = [
            ...
            'Frm_Logintoken' => GlobalFunction::find($get, 'Frm_Logintoken", "', '"'),
            'UserRandomNum'  => $rand,
        ];
    ...
    {
        $get = $this->request($this->modemUrl . '/getpage.gch?pid=1002&nextpage=' . 'manager_dev_conf_t.gch');
        ...
        $postdata = [
            ...
            'IF_ERRORTYPE'   => -1,
            'flag'           => 1,
            '_SESSION_TOKEN' => GlobalFunction::find($get, 'session_token = "', '";'),
        ];
        ...
        $request = $this->request($this->modemUrl . '/getpage.gch?pid=1002&nextpage=' . 'manager_dev_conf_t.gch', $options);
        $cek     = GlobalFunction::find($request, "flag','", "'");
...
class NetworkInterface extends Status
{
    ...
    {
        $request = $this->zte->request($this->zte->modemUrl . "/getpage.gch?pid=1002&nextpage=" . 'IPv46_status_wan2_if_t.gch');
...
/**
 * DOM !!!! IMPORTANT AND DONT CHANGE IT IF YOU DONT KNOW BOUT THIS
...
function str_get_html($str)
	{
		$dom = new simple_html_dom(null, $lowercase = true, $forceTagsClosed = true, $target_charset = 'UTF-8', $stripRN = true, $defaultBRText = "\r\n", $defaultSpanText = ' ');
	...
	return $dom->load($str, $lowercase = true, $stripRN = true);
}
...
class simple_html_dom
{
		...
		$options = 0)
	{
		...
		if($options & 1) { // Strip Smarty scripts
